...
import { Medication, PrescriptionStatus } from './types';
...
export default function App() {
  ...
  const filteredMedications = useMemo(() => {
    return MEDICATIONS.filter((med) => {
      ...
      if (searchQuery.trim().length > 0) {
        ...
        if (!matchesName && !matchesLatin && !matchesActive && !matchesSymptom && !matchesIndication && !matchesCategory) {
          return false;
        }
      }
  ...
  }, [searchQuery, selectedLetter, selectedCategory, selectedSymptom, selectedRxStatus]);
