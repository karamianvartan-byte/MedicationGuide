    ...
    <div className="p-5 sm:p-6 bg-white border border-[#E0DBCF] rounded-3xl shadow-2xs hover:shadow-md transition-all flex flex-col justify-between group">
      <div>
        ...
        <div className="flex justify-between items-start mb-3 gap-2">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <span
              ...
              title={medication.prescriptionStatus === 'Rx' ? t('byPrescription') : t('withoutPrescription')}
            >
              {medication.prescriptionStatus === 'Rx' ? t('rxTag') : t('otcTag')}
            </span>
            ...
            <button
              ...
              className="p-1.5 rounded-full hover:bg-[#F5F2EB] text-[#A09B8E] transition-colors cursor-pointer"
        ...
        <div className="space-y-2 mb-3">
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="text-[11px] font-bold text-[#A09B8E] uppercase tracking-wider">{t('releaseFormsLabel')}</span>
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {medication.isVED && (
              <span
                className="px-2.5 py-0.5 bg-[#E8F0E6] text-[#2E5E2A] rounded-md text-[11px] font-bold border border-[#BCD4B8] flex items-center gap-1"
                title={t('isVEDLabel')}
              >
                <span>🏛️</span>
            ...
            {medication.atcCode && (
              <span className="px-2 py-0.5 bg-[#F5F2EB] text-[#707060] rounded-md text-[10px] font-mono border border-[#E0DBCF]" title={t('atcLabel')}>
                АТХ: {medication.atcCode}
              </span>
            )}
          </div>
        </div>
        ...
        <div className="mb-3 p-3 bg-[#FDFCF8] border border-[#E0DBCF] rounded-2xl text-xs space-y-1">
          ...
          <div className="text-[#3E4238] leading-tight">
            <span className="font-semibold text-[#4A5D44]">{t('dosageAdults')} </span>
            {medication.dosage.adults}
          </div>
          {medication.dosage.children && (
            <div className="text-[#3E4238] leading-tight text-[11px]">
              <span className="font-semibold text-[#4A5D44]">{t('dosageChildren')} </span>
              {medication.dosage.children}
            </div>
          )}
        ...
        <div className="mb-3 border border-[#E0DBCF] rounded-2xl bg-[#F5F2EB] overflow-hidden text-xs">
          <button
            ...
            className="w-full px-3 py-2 flex items-center justify-between text-left font-bold text-[#4A5D44] hover:bg-[#E0DBCF]/30 transition-colors cursor-pointer"
          >
            <div className="flex items-center gap-1.5">
              <ShieldAlert className="w-3.5 h-3.5 text-[#B87A7A]" />
              <span>{t('sideEffects')} & {t('contraindications')}</span>
            </div>
            {showSafetyDetails ? (
          ...
          {showSafetyDetails ? (
            ...
            <div className="px-3 pb-2 pt-0.5 text-[11px] text-[#707060] flex flex-wrap gap-1">
              <span className="font-semibold text-[#B87A7A]">{t('contraindications')} </span>
      ...
      <div className="pt-3 border-t border-[#F5F2EB] flex items-center justify-between gap-2 mt-auto">
        <div className="text-[11px] sm:text-xs text-[#7B8E6A] font-semibold line-clamp-1">
          <span className="text-[#A09B8E] font-normal">{t('symptomsLabel')} </span>
          {medication.symptoms.slice(0, 3).join(', ')}
        </div>

        <button
          onClick={() => onOpenDetails(medication)}
          className="text-[#7B8E6A] font-bold text-xs sm:text-sm hover:underline flex items-center gap-0.5 shrink-0 group-hover:translate-x-0.5 transition-transform cursor-pointer"
        >
